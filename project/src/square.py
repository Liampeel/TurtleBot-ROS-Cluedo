...
class squares():
	...
	def rotateOnSpot(self, x, z):
		...
		cF = CluedoFinder()
		if self.rotation >= 0:
			while(current_angle <= (self.rotation)):
	...
	def publishCircle(self):
		...
		self.rotateOnSpot(0,angular)
		while not self.image_found:
			angular = 0.75*angular
			self.rotation = math.pi/2
			self.rotateOnSpot(0.1,angular)
			self.rotation = -2*math.pi
			self.rotateOnSpot(0,-0.2*math.pi)
	...
	def publish(self):
		self.rotation = 2*math.pi
		self.rotateOnSpot(0,0.1*math.pi)
		distance = 30
		count = 0
